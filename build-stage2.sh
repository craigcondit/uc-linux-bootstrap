...
time docker run --rm=true -v "$(pwd)/../stage3":/output:rw insideo/uc-stage2 \
        sh -c "tar cC /stage3 . > /output/stage3.tar"
time docker run --rm=true -v "$(pwd)/../packages-stage2":/output:rw insideo/uc-stage2 \
        sh -c "tar cC /packages . > /output/stage2-packages.tar"
